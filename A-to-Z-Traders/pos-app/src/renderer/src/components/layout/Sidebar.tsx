...
export function Sidebar(): JSX.Element {
  ...
  // Show only what this role may open, and drop a group left with no items.
  const groups = NAVIGATION.map((group) => ({
    ...group,
    items: group.items.filter((item) => canAccess(role, item.feature))
  ...
  return (
    <aside className="flex flex-col overflow-hidden border-r border-nav-line bg-nav-bg text-nav-ink">
      ...
      <div className="flex flex-col gap-3 border-t border-nav-line px-3 pt-3 pb-4">
        <div className="flex items-center gap-2 px-3 text-micro text-nav-ink-muted">
          <span className="truncate">
            Signed in as <span className="font-semibold text-nav-ink-strong">{whoLabel}</span>
          </span>
        </div>
        <button
          type="button"
